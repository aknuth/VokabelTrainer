...
export function getSourceCanvas(
  ...
  const normalizedParams = params.map((param) => Math.floor(normalizeDecimalNumber(param)))
  context.drawImage(image, normalizedParams[0],normalizedParams[1],normalizedParams[2],normalizedParams[3]);
  context.restore();
  return canvas;
}
